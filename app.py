...
def get_engine():
    return create_engine(
        "mysql+pymysql://root:phtQEhkQbTnCEzgBtrrrMpyuKXyuOrak@shinkansen.proxy.rlwy.net:24941/railway"
    )
...
def insert_vivienda(data):
    ...
    INSERT INTO viviendas (
        comunidad, ci, jefe_familia,
        num_habitantes, total_habitaciones,
        vm_intra, vm_peri,
...
if st.sidebar.button("🏠 Registrar Vivienda"):
...
if st.session_state.show_form:
    ...
    st.markdown("---")
    st.subheader("📝 Registrar Vivienda")

    with st.form("vivienda_form"):

        comunidad = st.selectbox(
            "Comunidad",
            ["", "Sucre", "Yamparáez", "Tarabuco", "Monteagudo"]
        )

        col1, col2 = st.columns(2)
        ci = col1.text_input("CI *")
        jefe = col2.text_input("Jefe de Familia *")

        col3, col4 = st.columns(2)
        habitantes = col3.number_input("N° Habitantes", 0)
        habitaciones = col4.number_input("Total Habitaciones", 0)

        col5, col6 = st.columns(2)
        vm_intra = col5.selectbox("VM-Intra", ["SI", "NO"])
        vm_peri = col6.selectbox("VM-Peri", ["SI", "NO"])

        st.markdown("### 📍 Coordenadas")

        col7, col8, col9 = st.columns(3)
        altitud = col7.number_input("Altitud", value=0.0)
        latitud = col8.number_input("Latitud", value=0.0)
        longitud = col9.number_input("Longitud", value=0.0)

        submitted = st.form_submit_button("Guardar")

        if submitted:
            if comunidad and ci and jefe:
                data = {
                    "comunidad": comunidad,
                    "ci": ci,
                    "jefe_familia": jefe,
                ...
                insert_vivienda(data)

                st.success("✅ Guardado en MySQL")
                st.session_state.show_form = False
                st.rerun()

            else:
                st.error("❌ Campos obligatorios faltan")
